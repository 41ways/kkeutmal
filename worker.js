/**
 * 끝말잇기 — Cloudflare Workers 무료 플랜용 서버.
 *
 *  - 화면 파일(public/)과 뜻풀이 조각(public/dict/)은 Cloudflare 정적 자산으로 나간다. 요청 수 한도에 잡히지 않는다.
...
import words from './dict/words.txt';
...
export class WordGame extends DurableObject {
  constructor(ctx, env) {
    super(ctx, env);
    this.socks = new Set();
    this.timer = null;
    ...
    // 사전(21만 낱말)은 Worker 가 켜질 때가 아니라 객체가 처음 깰 때 한 번 편다 — 켜는 시간 한도에 걸리지 않게
    if (!WordGame.checked) { game.loadDict(words); game.selfCheck(); WordGame.checked = true; }
  }
